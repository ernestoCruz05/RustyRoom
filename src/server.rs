...
use crate::resc::{Room, RoomState, Message, MessageType, UserStatus, User, RoomInfo};
...
impl AsyncChatServer {
    ...
    async fn load_rooms_from_db(&self) -> Result<(), sqlx::Error> {
        ...
        if rooms.is_empty() {
            ...
            if let Ok(_) = self.database.create_system_room(
                1, 
                "General", 
    ...
    async fn process_message_async(&self, message: Message, sender: mpsc::UnboundedSender<String>) -> Result<(), Box<dyn std::error::Error>> {
        let sender_username = if let Some(account) = self.database.get_user_by_id(message.sender_id).await? {
            if message.sender_id != 0 {
                let _ = self.database.set_user_online_status(message.sender_id, &account.username, true).await;
            }
            account.username
        } else {
            "Unknown".to_string()
        ...
        match &message.message_type {
            ...
            MessageType::CreateRoom { room_id, name, description, password } => {
                self.handle_create_room(*room_id, name, description, password, message.sender_id, sender).await?;
            }
            MessageType::RoomMessage { room_id, sender_username: _, content } => {
                self.handle_room_message(*room_id, content, &sender_username, message.sender_id).await?;
            }
            MessageType::PrivateMessage { to_user_id, sender_username: _, content } => {
                self.handle_private_message(*to_user_id, content, &sender_username, message.sender_id).await?;
            }
            MessageType::Join { room_id, password } => {
                self.handle_join_room(*room_id, password, message.sender_id, sender).await?;
            }
            MessageType::Leave { room_id } => {
                self.handle_leave_room(*room_id, message.sender_id, sender).await?;
            }
            _ => {
    ...
    async fn handle_register(&self, username: &str, password: &str, sender: mpsc::UnboundedSender<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn handle_login(&self, username: &str, password: &str, sender: mpsc::UnboundedSender<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn handle_create_room(&self, room_id: u16, name: &str, description: &str, password: &Option<String>, creator_id: u16, sender: mpsc::UnboundedSender<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn handle_room_message(&self, room_id: u16, content: &str, sender_username: &str, sender_id: u16) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn handle_private_message(&self, to_user_id: u16, content: &str, sender_username: &str, sender_id: u16) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn handle_join_room(&self, room_id: u16, password: &Option<String>, user_id: u16, sender: mpsc::UnboundedSender<String>) -> Result<(), Box<dyn std::error::Error>> {
            ...
            if can_join {
                ...
                match self.database.get_recent_messages(room_id).await {
                    Ok(messages) => {
                        ...
                        let separator = Message::new(0, MessageType::RoomMessage {
                            room_id,
    ...
    async fn handle_leave_room(&self, room_id: u16, user_id: u16, sender: mpsc::UnboundedSender<String>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let leave_notification = Message::new(0, MessageType::RoomMessage {
            room_id,
            sender_username: "System".to_string(),
    ...
    fn send_to_sender(&self, sender: &mpsc::UnboundedSender<String>, message: Message) -> Result<(), std::io::Error> {
        if let Ok(json) = message.to_json_file() {
            sender.send(json).map_err(|_| std::io::Error::new(std::io::ErrorKind::BrokenPipe, "Channel closed"))?;
        }
        Ok(())
    }
    ...
    async fn cleanup_disconnected_users(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for user_id in disconnected_users {
            ...
            if let Some(account) = self.database.get_user_by_id(user_id).await.unwrap_or(None) {
                let _ = self.database.set_user_online_status(user_id, &account.username, false).await;
                println!("User {} ({}) automatically marked as offline due to disconnection", account.username, user_id);
            }
        }
