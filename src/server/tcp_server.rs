...
use crate::core::Room;
use crate::protocol::message::{Message, MessageType, UserStatus};
...
pub struct AsyncChatServer {
	users: Arc<RwLock<HashMap<u16, crate::core::Account>>>,
...
impl AsyncChatServer {
	...
	pub async fn start_async_server(database_url: &str, address: &str) -> Result<(), Box<dyn std::error::Error>> {
		...
		if let Err(e) = server.load_rooms_from_db().await {
			eprintln!("  Warning: Could not load existing rooms from database: {}", e);
		}
		...
		let write_task = tokio::spawn(async move {
			while let Some(message) = rx.recv().await {
				if let Err(e) = write_half.write_all(message.as_bytes()).await {
					eprintln!("Failed to write to client: {} - connection likely broken", e);
					break;
				}
				if let Err(e) = write_half.write_all(b"\n").await {
					eprintln!("Failed to write newline to client: {} - connection likely broken", e);
					break;
				}
				if let Err(e) = write_half.flush().await {
					eprintln!("Failed to flush client stream: {} - connection likely broken", e);
					break;
				}
			}
		});
	...
	async fn process_message_async(
		...
		match &message.message_type {
			...
			MessageType::CreateRoom {
				...
				self
					.handle_create_room(*room_id, name, description, password, message.sender_id, sender)
					.await?;
			}
			MessageType::RoomMessage { room_id, sender_username: _, content } => {
				self
					.handle_room_message(*room_id, content, &sender_username, message.sender_id)
					.await?;
			}
			MessageType::PrivateMessage { to_user_id, sender_username: _, content } => {
				self
					.handle_private_message(*to_user_id, content, &sender_username, message.sender_id)
					.await?;
			}
			MessageType::Join { room_id, password } => {
				self
					.handle_join_room(*room_id, password, message.sender_id, sender)
					.await?;
			}
			MessageType::Leave { room_id } => {
				self
					.handle_leave_room(*room_id, message.sender_id, sender)
					.await?;
			}
			_ => {}
		}
		...
		match self.database.create_user(username, &password_hash).await {
			Ok(user_id) => {
				...
				let response = Message::new(
					0,
					MessageType::AuthSuccess { user_id, message: format!("Registration successful! Welcome, {}", username) },
				);
			...
			Err(_) => {
				let response = Message::new(
					0,
					MessageType::AuthFailure { reason: "Username already exists".to_string() },
				);
				self.send_to_sender(&sender, response)?;
			}
		}
	...
	async fn handle_login(
		...
		if let Some(account) = self.database.get_user_by_username(username).await? {
			...
			if account.hash == password_hash {
				...
				let response = Message::new(
					0,
					MessageType::AuthSuccess { user_id: account.user_id, message: format!("Login successful! Welcome back, {}", username) },
				);
				self.send_to_sender(&sender, response)?;

				self.broadcast_user_list_update().await?;
			} else {
				let response = Message::new(0, MessageType::AuthFailure { reason: "Invalid password".to_string() });
				self.send_to_sender(&sender, response)?;
			}
		} else {
			let response = Message::new(0, MessageType::AuthFailure { reason: "Account does not exist".to_string() });
			self.send_to_sender(&sender, response)?;
		}
	...
	async fn handle_list_rooms(&self, sender: mpsc::UnboundedSender<String>) -> Result<(), Box<dyn std::error::Error>> {
	...
	async fn handle_user_list_request(&self, sender: mpsc::UnboundedSender<String>) -> Result<(), Box<dyn std::error::Error>> {
	...
	async fn handle_create_room(
		...
		match self
			.database
			.create_room(room_id, name, description, &room_state, password_hash.as_deref(), creator_id)
			.await
		{
			Ok(_) => {
				let mut rooms = self.rooms.write().await;
				let room = if let Some(pass_hash) = password_hash {
					...
					Room::new(room_id, name.to_string(), description.to_string(), crate::core::RoomState::Open)
				};
				rooms.insert(room_id, room);

				let response = Message::new(
					0,
					MessageType::RoomCreated { room_id, name: name.to_string(), description: description.to_string() },
				);
				self.send_to_sender(&sender, response)?;
			}
			Err(_) => {
				let response = Message::new(
					0,
					MessageType::ServerResponse { success: false, content: format!("Failed to create room {}", room_id) },
				);
				self.send_to_sender(&sender, response)?;
			}
		}
	...
	async fn handle_room_message(
		...
		let message = Message::new(
			sender_id,
			MessageType::RoomMessage { room_id, sender_username: sender_username.to_string(), content: content.to_string() },
		);
	...
	async fn handle_private_message(
		...
		let message = Message::new(
			sender_id,
			MessageType::PrivateMessage { to_user_id, sender_username: sender_username.to_string(), content: content.to_string() },
		);
	...
	async fn handle_join_room(
		...
		if let Some(room) = self.database.get_room(room_id).await? {
			...
			if can_join {
				...
				let response = Message::new(
					0,
					MessageType::RoomJoined { room_id, name: room.name.clone(), description: room.description.clone() },
				);
				self.send_to_sender(&sender, response)?;
				...
				match self.database.get_recent_messages(room_id).await {
					Ok(messages) => {
						for stored_message in messages {
							let history_message = Message::new(
								...
								MessageType::RoomMessage {
									...
									content: format!("[{}] {}", stored_message.timestamp.format("%H:%M"), stored_message.content),
								},
							);
						...
						let separator = Message::new(
							0,
							MessageType::RoomMessage { room_id, sender_username: "System".to_string(), content: "--- End of message history ---".to_string() },
						);
				...
				let join_notification = Message::new(
					0,
					MessageType::RoomMessage { room_id, sender_username: "System".to_string(), content: format!("{} joined the room", username) },
				);
				...
				let response = Message::new(
					0,
					MessageType::ServerResponse { success: false, content: "Invalid room password".to_string() },
				);
				self.send_to_sender(&sender, response)?;
			}
		} else {
	...
	async fn handle_leave_room(
		...
		let response = Message::new(0, MessageType::ServerResponse { success: true, content: format!("Left room {}", room_id) });
		self.send_to_sender(&sender, response)?;
		...
		let leave_notification = Message::new(
			0,
			MessageType::RoomMessage { room_id, sender_username: "System".to_string(), content: format!("{} left the room", username) },
		);
	...
	async fn send_to_user(&self, user_id: u16, message: Message) -> Result<(), Box<dyn std::error::Error>> {
	...
	fn send_to_sender(&self, sender: &mpsc::UnboundedSender<String>, message: Message) -> Result<(), std::io::Error> {
	...
	async fn broadcast_user_list_update(&self) -> Result<(), Box<dyn std::error::Error>> {
		...
		let message = Message::new(0, MessageType::UserListUpdate { users: user_statuses });
