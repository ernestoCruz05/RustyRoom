...
impl ChatClient {
	pub fn new(server_address: String, username: String) -> Self {
		ChatClient { server_address, user_id: 0, username, authenticated: false }
	}
	...
	fn display_message(message: Message) {
		match message.message_type {
			...
			MessageType::ServerResponse { success, content } => {
				if success { println!("[Server]: {}", content); } else { println!("[Server Error]: {}", content); }
			}
			MessageType::RoomCreated { room_id, name, description } => {
				println!("[SUCCESS] Room '{}' (ID: {}) created successfully!", name, room_id);
				println!("         Description: {}", description);
			}
			MessageType::RoomJoined { room_id, name, description: _ } => {
			...
			MessageType::RoomNotFound { room_id } => {
				...
				println!("       Would you like to create it? Use:\n       /create {} <name> <description> [password]", room_id);
			}
			...
			MessageType::RoomList { rooms } => {
				...
				for room in rooms { println!("  [{}] {} - {}", room.id, room.name, room.description); }
			}
			...
			MessageType::UserListUpdate { users } => {
				...
				for user in users { let status = if user.is_online { "●" } else { "○" }; println!("  {} {}", status, user.username); }
			}
			MessageType::RequestUserList => {
				println!("[Debug] Received RequestUserList");
			}
			MessageType::VoiceChannelJoin { .. }
			| MessageType::VoiceChannelLeave { .. }
			| MessageType::VoiceChannelJoined { .. }
			| MessageType::VoiceChannelLeft { .. }
			| MessageType::VoiceUserSpeaking { .. }
			| MessageType::VoiceUserJoined { .. }
			| MessageType::VoiceUserLeft { .. } => {
				println!("[Voice] Control message received");
			}
			_ => {
			}
		}
	...
	fn handle_user_input(&mut self, stream: &mut TcpStream, client_clone: Arc<Mutex<ChatClient>>) -> io::Result<()> {
		...
		for line in stdin.lock().lines() {
			...
			let is_authenticated = { client_clone.lock().unwrap().authenticated };
	...
	fn parse_command(&self, input: &str, is_authenticated: bool) -> Option<Message> {
		...
		match parts.get(0)? {
			&"/register" => {
				if is_authenticated { println!("You are already logged in!"); return None; }
				if parts.len() < 3 { println!("Usage: /register <username> <password>"); return None; }
			...
			&"/login" => {
				if is_authenticated { println!("You are already logged in!"); return None; }
				if parts.len() < 3 { println!("Usage: /login <username> <password>"); return None; }
				let username = parts[1].to_string();
				let password = parts[2].to_string();
				Some(Message::new(0, MessageType::Login { username, password }))
			}
			&"/join" => {
				if !is_authenticated { println!("Please login or register first!"); return None; }
				if parts.len() < 2 { println!("Usage: /join <room_id> [password]"); return None; }
				let room_id: u16 = parts.get(1)?.parse().ok()?;
				let password = if parts.len() > 2 { Some(parts[2].to_string()) } else { None };
				Some(Message::new(self.user_id, MessageType::Join { room_id, password }))
			}
			&"/leave" => {
				if !is_authenticated { println!("Please login or register first!"); return None; }
				if parts.len() < 2 { println!("Usage: /leave <room_id>"); return None; }
				let room_id: u16 = parts.get(1)?.parse().ok()?;
			...
			&"/create" => {
				...
				if args.len() < 4 { println!("Usage: /create <room_id> <name> <description> [password]"); return None; }
				let room_id: u16 = args.get(1)?.parse().ok()?;
				let name = args.get(2)?.clone();
				let description = args.get(3)?.clone();
				let password = if args.len() > 4 { Some(args.get(4)?.clone()) } else { None };
				Some(Message::new(self.user_id, MessageType::CreateRoom { room_id, name, description, password }))
			}
			&"/msg" => {
				if !is_authenticated { println!("Please login or register first!"); return None; }
				if parts.len() < 3 { println!("Usage: /msg <room_id> <message>"); return None; }
				let room_id: u16 = parts.get(1)?.parse().ok()?;
			...
			&"/private" => {
				if !is_authenticated { println!("Please login or register first!"); return None; }
				if parts.len() < 3 { println!("Usage: /private <user_id> <message>"); return None; }
			...
			_ => {
				...
				if !is_authenticated { println!("Available commands: /register, /login, /quit"); }
				else { println!("Available commands: /join, /leave, /create, /msg, /private, /quit"); }
				None
			}
		}
	}
	...
	fn parse_quoted_args(input: &str) -> Option<Vec<String>> {
		...
		while let Some(ch) = chars.next() {
			match ch {
				'"' => { in_quotes = !in_quotes; }
				' ' if !in_quotes => {
					if !current_arg.is_empty() { args.push(current_arg.trim().to_string()); current_arg.clear(); }
				}
	...
	fn show_auth_commands() {
		println!("Welcome to RustyRoom CLI!");
	...
	fn show_chat_commands() {
		println!("\n=== Authenticated ===");
		println!("  /join <room_id> [password]");
		println!("  /leave <room_id>");
		println!("  /create <room_id> <name> <description> [password]");
		println!("  /msg <room_id> <message>");
