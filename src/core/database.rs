/*
 Database module ported from old project.
*/

use crate::core::{Account, Room, RoomState};
use crate::protocol::message::RoomInfo;
...
impl Database {
	...
	pub async fn create_user(&self, username: &str, password_hash: &str) -> Result<u16, sqlx::Error> {
		let result = sqlx::query("INSERT INTO users (username, password_hash) VALUES (?, ?) RETURNING id")
			.bind(username)
			.bind(password_hash)
			.fetch_one(&self.pool)
			.await?;
	...
	pub async fn create_room(
		...
		sqlx::query("INSERT INTO rooms (id, name, description, state, password_hash, created_by) VALUES (?, ?, ?, ?, ?, ?)")
			.bind(id as i64)
			.bind(name)
			.bind(description)
			.bind(state_str)
	...
	pub async fn create_system_room(&self, id: u16, name: &str, description: &str) -> Result<(), sqlx::Error> {
		sqlx::query("INSERT INTO rooms (id, name, description, state, password_hash, created_by) VALUES (?, ?, ?, ?, ?, NULL)")
			.bind(id as i64)
			.bind(name)
			.bind(description)
			.bind("Open")
	...
	pub async fn get_room(&self, room_id: u16) -> Result<Option<Room>, sqlx::Error> {
		let row = sqlx::query("SELECT id, name, description, state, password_hash FROM rooms WHERE id = ?")
			.bind(room_id as i64)
			.fetch_optional(&self.pool)
			.await?;
		...
		match row {
			Some(row) => {
				...
				let room = if let Some(password_hash) = row.get::<Option<String>, _>("password_hash") {
					Room::new_with_password(row.get::<i64, _>("id") as u16, row.get("name"), row.get("description"), password_hash)
				} else {
					Room::new(row.get::<i64, _>("id") as u16, row.get("name"), row.get("description"), state)
				};
	...
	pub async fn get_all_rooms(&self) -> Result<Vec<RoomInfo>, sqlx::Error> {
		let rows = sqlx::query("SELECT id, name, description, password_hash FROM rooms ORDER BY name")
			.fetch_all(&self.pool)
			.await?;
	...
	pub async fn add_user_to_room(&self, user_id: u16, room_id: u16) -> Result<(), sqlx::Error> {
		sqlx::query("INSERT OR IGNORE INTO room_memberships (user_id, room_id) VALUES (?, ?)")
			.bind(user_id as i64)
			.bind(room_id as i64)
			.execute(&self.pool)
			.await?;
	...
	pub async fn remove_user_from_room(&self, user_id: u16, room_id: u16) -> Result<(), sqlx::Error> {
		sqlx::query("DELETE FROM room_memberships WHERE user_id = ? AND room_id = ?")
			.bind(user_id as i64)
			.bind(room_id as i64)
			.execute(&self.pool)
			.await?;
	...
	pub async fn get_room_members(&self, room_id: u16) -> Result<Vec<u16>, sqlx::Error> {
		let rows = sqlx::query("SELECT user_id FROM room_memberships WHERE room_id = ?")
			.bind(room_id as i64)
			.fetch_all(&self.pool)
			.await?;

		let members: Vec<u16> = rows.into_iter().map(|row| row.get::<i64, _>("user_id") as u16).collect();
		Ok(members)
	}
	...
	pub async fn load_all_accounts(&self) -> Result<HashMap<String, Account>, sqlx::Error> {
		...
		for row in rows {
			let account = Account::new(row.get::<String, _>("username").clone(), row.get("password_hash"), row.get::<i64, _>("id") as u16);
	...
	pub async fn load_all_rooms(&self) -> Result<HashMap<u16, Room>, sqlx::Error> {
		let rows = sqlx::query("SELECT id, name, description, state, password_hash FROM rooms")
			.fetch_all(&self.pool)
			.await?;
